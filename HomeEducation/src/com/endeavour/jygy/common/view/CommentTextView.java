    ...
    private void initView() {
        ...
        reviewSpan = new ClickableSpan() {
            ...
            public void onClick(View widget) {
                DynamicDiscuss model1 = (DynamicDiscuss) widget.getTag();
//                Toast.makeText(mContext, model1.getReUserName(), Toast.LENGTH_SHORT).show();
            }
...
//                return result;
     ...
     * @return
     */
    public void setReply(DynamicDiscuss model) {
        setText("");
        setTag(model);
        String reviewName = model.getReUserName();
        SpannableStringBuilder stylesBuilder;
        String str;
        if (!TextUtils.isEmpty(model.getBeUserName())) {
            String replyName = model.getBeUserName();
            if (TextUtils.isEmpty(replyName)) {
                replyName = " ";
            }
            str = reviewName + "回复" + replyName + "：" + Unicoder.unicode2Emoji(model.getCommentContent());
            ...
            str = reviewName + "：" + Unicoder.unicode2Emoji(model.getCommentContent());
            int reviewStart = str.indexOf(reviewName);
            stylesBuilder = new SpannableStringBuilder(str);
            try {
                stylesBuilder.setSpan(reviewSpan, reviewStart, reviewStart + reviewName.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
