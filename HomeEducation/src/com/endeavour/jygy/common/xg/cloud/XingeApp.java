package com.endeavour.jygy.common.xg.cloud;
...
public class XingeApp {
	...
	public JSONObject pushSingleDevice(String deviceToken, Message message)
		...
		if (!message.isValid()) {
			return new JSONObject(
					"{'ret_code':-1,'err_msg':'message invalid!'}");
		}
	...
	public JSONObject pushSingleDevice(String deviceToken, MessageIOS message,
			int environment) throws JSONException {
		if (!message.isValid()) {
			return new JSONObject(
					"{'ret_code':-1,'err_msg':'message invalid!'}");
		}
	...
	public JSONObject pushAccountList(int deviceType, List<String> accountList,
			Message message) throws JSONException {
		if (!message.isValid()) {
			return new JSONObject(
					"{'ret_code':-1,'err_msg':'message invalid!'}");
	...
	public JSONObject pushSingleAccount(int deviceType, String account,
			MessageIOS message, int environment) throws JSONException {
		if (!message.isValid()) {
			return new JSONObject(
					"{'ret_code':-1,'err_msg':'message invalid!'}");
		}
	...
	public JSONObject pushTags(int deviceType, List<String> tagList,
			String tagOp, Message message) throws JSONException {
	...
	public JSONObject pushTags(int deviceType, List<String> tagList,
			String tagOp, MessageIOS message, int environment)
			throws JSONException {
		if (!message.isValid() || tagList.size() == 0
				|| (!tagOp.equals("AND") && !tagOp.equals("OR"))) {
			return new JSONObject("{'ret_code':-1,'err_msg':'param invalid!'}");
	...
	protected JSONObject callRestful(String url, Map<String, Object> params)
		...
		if (TextUtils.isEmpty(sign))
			return new JSONObject(
					"{\"ret_code\":-1,\"err_msg\":\"generateSign error\"}");
		...
		try {
			...
			// e.printStackTrace();
			jsonRet = new JSONObject(
					"{\"ret_code\":-1,\"err_msg\":\"call restful error\"}");
		}
		return jsonRet;
	}
	...
	public JSONObject BatchSetTag(List<TagTokenPair> tagTokenPairs)
		...
		for (TagTokenPair pair : tagTokenPairs) {
			if (!this.ValidateToken(pair.token)) {
				String returnMsgJsonStr = String.format(
