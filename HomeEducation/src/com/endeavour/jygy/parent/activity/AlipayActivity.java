...
//        aliPayBody.setFeeId(getPayInfoResp.getId());
//        aliPayBody.setUserId(AppConfigHelper.getConfig(AppConfigDef.studentID));
//        AlipayNotifyReq alipayNotifyReq = new AlipayNotifyReq();
//        alipayNotifyReq.setBody(aliPayBody);
//        NetRequest.getInstance().addRequest(alipayNotifyReq, new BaseRequest.ResponseListener() {
//            @Override
...
//        });
    ...
    public void pay() {
        if (TextUtils.isEmpty(PARTNER) || TextUtils.isEmpty(RSA_PRIVATE) || TextUtils.isEmpty(SELLER)) {
            progresser.showError("支付参数不完整", false);
            return;
        }
        PayBody payBody = new PayBody();
        payBody.setFeeId(getPayInfoResp.getId());
        payBody.setUserId(AppConfigHelper.getConfig(AppConfigDef.studentID));
        String body = JSONObject.toJSONString(payBody);
        String orderInfo = getOrderInfo(getPayInfoResp.getFeeName(), body, Calculater.formotFen(getPayInfoResp.getFeeAmount()));
    ...
    private String getOrderInfo(String subject, String body, String price) {
        ...
        // 服务器异步通知页面路径
        orderInfo += "&notify_url=" + "\"" + "http://" + Constants.ALIPAY_NOTIFY_IP + (TextUtils.isEmpty(Constants.ALIPAY_NOTIFY_PORT) ? "" : ":" + Constants.ALIPAY_NOTIFY_PORT) + "/jygy-service/v1_0/common/alipayNotify" + "\"";
